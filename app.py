                ...
                for col in selected_keys:
                    ...
                    if all_points:
                        ...
                        if not final_series.empty:
                            ...
                            # 1. Ép kiểu datetime cho chắc chắn
                            plot_data['TG'] = pd.to_datetime(plot_data['TG'])
                            
                            # 2. Tính toán Hàng rào và Khung nhìn
                            min_time = plot_data['TG'].min()
                            max_time = plot_data['TG'].max()
                            time_buffer = pd.Timedelta(minutes=5)
                            
                            # Dùng chuẩn ISO 8601 an toàn cho Plotly
                            bound_min = (min_time - time_buffer).strftime('%Y-%m-%dT%H:%M:%S')
                            bound_max = (max_time + time_buffer).strftime('%Y-%m-%dT%H:%M:%S')
