...
with tab_future:
    ...
    with left_col:
        ...
        with st.container(border=True):
            ...
            for block, (vmin, vmax) in list(st.session_state.current_matrix.items()):
...
# --------------------------------------------------------
# 📁 TAB 2: PHÂN TÍCH FILE IOT (DẢI CỐ ĐỊNH)
# --------------------------------------------------------
with tab_past:
    ...
    if uploaded_file:
        try:
            ...
            if df_calc.empty:
                ...
                from analytics import calculate_static_plant_stress
