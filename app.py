...
# SỬA LỖI CHUỖI DÀI PYTHON 3.14+: Chuyển đổi sang st.html() an toàn, giữ nguyên các Class CSS gốc của bạn
st.html(
    """
    <style>
...
# CHIA TÁCH THÀNH 2 MỤC ĐỘC LẬP THEO ĐÚNG YÊU CẦU CỦA BẠN
tab1, tab2 = st.tabs(["MÔ PHỎNG & XEM REALTIME", "📂 ĐỌC VÀ PHÂN TÍCH FILE JSON"])

# Hàm tiện ích lấy ma trận sinh học gốc từ file của bạn
...
# =====================================================================
# TAG 1: MÔ PHỎNG & XEM REALTIME (GIỮ NGUYÊN VẸN 100% GIAO DIỆN & CHỨC NĂNG GỐC)
# =====================================================================
with tab1:
    ...
    with col_left:
        ...
        sim_hour = st.slider("Mô phỏng giờ trong ngày:", 0, 23, current_time.hour, key="sl_hour_t1")
        sim_minute = st.slider("Mô phỏng phút:", 0, 59, current_time.minute, key="sl_min_t1")
    ...
    with col_right:
        ...
        # Khởi tạo dữ liệu tuần hoàn 24h và thêm trường 'Buổi' để triệt tiêu hoàn toàn lỗi TypeError khi groupby
        records_24h = []
        ...
        try:
            ...
            st.error(f"⚠️ Hệ thống đang đồng bộ cấu trúc bảng phân tích chu kỳ. Chi tiết: {str(report_err)}")
...
# =====================================================================
# TAG 2: ĐỌC VÀ PHÂN TÍCH FILE JSON (MỤC KHÁC TÁCH BIỆT RIÊNG)
# =====================================================================
with tab2:
    ...
    uploaded_json = st.file_uploader("Chọn file dữ liệu IoT (.json):", type=["json"], key="uploader_tab2_json")
                    ...
                    if st.button("📤 Gửi Toàn Bộ Báo Cáo JSON Qua Telegram", type="primary", key="btn_send_json_tele"):
                        if TELE_TOKEN and TELE_CHAT_ID:
                            ...
                            if send_telegram_message(TELE_TOKEN, TELE_CHAT_ID, json_tele_msg):
                                ...
                                st.error("❌ Truyền dữ liệu API sang máy chủ Telegram thất bại.")
                except Exception as json_report_err:
                    st.error(f"⚠️ Cấu trúc dữ liệu bảng không tương thích. Lỗi: {str(json_report_err)}")
            
        except Exception as err:
            st.error(f"❌ Không thể đọc cấu trúc tệp JSON này. Vui lòng kiểm tra lại cấu trúc file. Lỗi: {str(err)}")
