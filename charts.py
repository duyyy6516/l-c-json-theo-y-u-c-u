...
def draw_vpd_chart(df, vpd_min, vpd_max):
    ...
    # 2. Vẽ đường dữ liệu VPD dạng Line (Tránh bị bí giống như cột Bar)
    ...
    # 3. Cấu hình trục giúp giãn cách thoáng đãng, xoay nghiêng chữ 45 độ
    chart = (band + line).properties(height=300).configure_axisX(
        labelAngle=-45,
        labelOverlap="hide",  # Tự ẩn chữ thừa nếu quá dày để tránh đè nhau
        labelPadding=8
    ).configure_axisY(
        labelPadding=8
    ).configure_view(
        strokeOpacity=0       # Loại bỏ khung viền thô cứng giúp mở rộng biểu đồ
    )
    return chart
...
def draw_temperature_chart(df):
    ...
    chart = alt.Chart(df).mark_line(point=True, color='#FF4B4B', strokeWidth=2.5).encode(
        ...
        labelAngle=-45, labelOverlap="hide", labelPadding=8
...
def draw_combined_chart(df):
    ...
    # Chuyển đổi dữ liệu để vẽ biểu đồ tổ hợp nhiều đường thoáng đãng
