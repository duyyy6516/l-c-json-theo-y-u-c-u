...
def draw_temperature_chart(df):
    ...
    chart = alt.Chart(df).mark_line(color="#FF4B4B", point=True).encode(
        x=alt.X('Hiển thị Giờ:N', title="Mốc thời gian", sort=None), # Chuyển :O sang :N giúp giãn khoảng cách
        ...
        labelAngle=-45,
        labelOverlap="greedy",  # Thuật toán tự động lọc bỏ các mốc giờ trùng lặp khi file quá dày
        labelPadding=8
    ).configure_view(
        strokeOpacity=0       # Xóa bỏ khung viền thô cứng để biểu đồ mở rộng tự nhiên
    )
    return chart
...
def draw_vpd_chart(df, vpd_min, vpd_max):
    ...
    # Đường đồ thị chính màu xanh lá cây đậm (Dùng :N thay vì :O để tránh bị nén dữ liệu)
    ...
    chart = (rect_blue + rect_red + line_vpd).properties(height=250).interactive().configure_axisX(
        labelAngle=-45,
        labelOverlap="greedy",
        labelPadding=8
    ).configure_view(
...
def draw_combined_chart(df):
    ...
    chart = alt.layer(weather_layer, line_v).properties(height=250).resolve_scale(
        y='independent'
